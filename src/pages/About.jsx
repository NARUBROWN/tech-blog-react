    ...
    const professionalProjects = [
        {
            ...
            role: 'AI Engineer',
            org: '(주)셈웨어',
            summary: '교육과정 PDF와 교과서를 분석해 온톨로지를 구축하고, 시험 문제와 학생 응답 데이터를 연결해 선수학습 결손·개념 혼동·상위 개념 미숙달을 진단하는 학습 분석 시스템입니다.',
            techs: ['FastAPI', 'Apache Jena Fuseki', 'RDF-star', 'SQLite', 'LLM', 'OCR', 'Ontology', 'Knowledge Graph'],
            problems: [
                '교육과정 성취기준 문서에는 무엇을 가르쳐야 하는지는 적혀 있지만, 개념 사이의 위계나 선수학습 관계는 잘 드러나지 않아 학생이 특정 문제를 틀렸을 때 원인을 구조적으로 추적하기 어려웠음',
                '교육과정 PDF에서 많은 개념과 관계를 뽑아내더라도 AI 출력만 그대로 믿으면 누락되거나 잘못 연결된 관계가 섞일 수 있어, 온톨로지 전체를 바로 반영하기 어려웠음'
            ],
            solutions: [
                '온톨로지 구축을 두 단계로 나눠, 개념 체계와 관계의 기준은 사람이 먼저 설계하고 실제 개념 데이터는 AI가 교육과정 문서에서 추출하도록 구성했음. 이를 통해 추론 가능한 기본 구조를 먼저 안정적으로 마련했음',
                '교육과정 PDF는 학년군 단위로 나눠 병렬 처리하고, 성취기준을 잘게 쪼개 개념을 뽑은 뒤 개념 사이 관계를 다시 분석하도록 파이프라인을 설계했음. 관계마다 신뢰도와 적용 학년 정보를 함께 저장해 이후 진단에 활용할 수 있게 했음',
                '문서만으로는 위계 관계를 복원하기 어렵다고 판단해 교과서를 추가 근거로 사용했음. 교과서의 제목과 본문을 기존 개념과 연결한 뒤, 실제 문장에 근거가 있는 경우에만 새로운 위계 관계를 제안하도록 만들었음',
                'AI가 만든 결과를 바로 반영하지 않도록 잘못된 순환 관계, 자기 참조, 근거 없는 제안, 존재하지 않는 개념 연결 등을 자동으로 걸러냈고, 교사 검토와 승인 이후에만 반영되도록 해 운영 안정성을 확보했음'
            ],
            results: [
                '교육과정 문서를 추론 가능한 지식 그래프로 바꾸고, 시험 문제를 개념에 연결해 학생 오답을 선수학습 결손과 개념 혼동 관점에서 해석할 수 있는 기반을 마련했음',
                '교육과정 PDF 성취기준 코드 추출율을 기준으로 수학 2015는 41.3%에서 100.0%, 과학 2022는 36.9%에서 99.8%까지 끌어올렸고, 수학 2022와 과학 2015도 각각 88.0%, 92.5% 수준까지 확보했음',
                '온톨로지 시드 품질 벤치마크에서 종합 89.8점을 기록했고, 커버리지 100%, 선수학습 관계 사이클 0건, 규칙 위반 1건 수준으로 구조적 안정성을 검증했음',
                'AI 추출, 교과서 근거 확인, 교사 승인, 롤백 가능한 반영 절차를 함께 두어 정확성과 운영 안정성을 모두 확보했음'
            ]
        },
        {
            date: '2025.02 ~',
            title: '카스탈리아 (Castalia)',
            role: 'AI Engineer',
            org: '(주)셈웨어',
            summary: 'AI 기반 교육용 학습지 자동 생성 플랫폼으로, 교육과정 PDF를 계층적으로 분석해 커리큘럼 구조를 추출하고 워크북을 자동 생성·렌더링합니다.',
            techs: ['FastAPI', 'PostgreSQL', 'SQLAlchemy', 'ChromaDB', 'PyMuPDF', 'Playwright', 'OpenRouter', 'RAG'],
            problems: [
                '수십~수백 페이지에 이르는 교육과정 PDF에서 성취기준 코드를 하나도 빠뜨리지 않고 뽑아내야 했기 때문에, 일반적인 검색 기반 방식만으로는 필요한 정확도를 맞추기 어려웠음',
                '성취기준, 핵심 아이디어, 교수학습 방법이 한 문서 안에 섞여 있어 이를 학년별 커리큘럼 구조로 다시 정리하는 과정이 복잡했고, 한 번의 AI 호출만으로는 결과 품질이 일정하지 않았음',
                '같은 키워드가 여러 학년과 과목에서 반복되다 보니 단순 유사도 검색만 사용하면 다른 학년이나 과목 내용이 함께 섞이는 문제가 있었음'
            ],
            solutions: [
                '교육과정 PDF를 학년군 단위로 나눠 병렬 처리하는 구조를 설계해 긴 문서도 안정적으로 분석할 수 있게 했고, 처리 중간마다 누락 여부를 점검해 빠진 구간만 다시 보완하도록 만들었음',
                '단계를 한 번에 처리하지 않고, 내용 추출 → 코드 연결 → 세부 내용 생성처럼 여러 단계로 나눠 AI가 맡는 작업을 분리함으로써 학년 배정과 커리큘럼 정리 품질을 안정화했음',
                '검색 단계에서는 단순 유사도만 보지 않고 학년, 과목, 영역 같은 문맥 정보를 함께 사용해 다른 학년 내용이 섞이지 않도록 제어했음',
                'PDF 추출부터 데이터 저장, 워크북 파일 생성과 렌더링까지 하나의 자동화된 흐름으로 연결해 교사가 문서를 올리면 결과물을 바로 만들 수 있는 구조를 구현했음'
            ],
            results: [
                '교육과정 원문 누락을 최소화하면서도 긴 문서를 안정적으로 다룰 수 있는 생성 파이프라인 기반을 마련했음',
                '학년과 과목 문맥이 섞이지 않도록 구조화해 워크북 생성 정확도를 높였음',
                '수학 2015 기준 성취기준 코드 추출 정확도를 41.3%에서 100.0%까지 개선했고, 동일 문서에서 토큰 사용량은 16.6%, 비용은 34.8% 절감했음',
                '4개 교육과정 PDF 전체 기준으로도 비용을 2,946원에서 631원 수준까지 약 79% 줄이면서, 전체 처리 시간 약 23분의 자동화된 생성 파이프라인을 구축했음',
                '교사의 PDF 업로드부터 커리큘럼 분석, 워크북 생성, 파일 렌더링까지 이어지는 자동화된 AI 워크플로우를 구현했음'
            ]
        },
        {
